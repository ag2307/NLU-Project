	...
	with torch.no_grad():
		for idx in range(sentences):
			...
			for i in range(100):
				...
				cur_ids = torch.cat([cur_ids.to(device), torch.ones((1,1)).long().to(device) * next_token_id], dim = 1)
...
if __name__ == '__main__':
	...
	MODEL.to(device)
	labels = ['INDIRECT-DOWNREGULATOR', 'AGONIST-INHIBITOR', 'PRODUCT-OF', 'DOWNREGULATOR', 'ACTIVATOR', 'INDIRECT-UPREGULATOR', 'INHIBITOR', 'AGONIST-ACTIVATOR', 'SUBSTRATE_PRODUCT-OF', 'ANTAGONIST', 'AGONIST', 'UPREGULATOR', 'SUBSTRATE']
	for LABEL in labels:
